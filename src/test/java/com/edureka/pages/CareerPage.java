...
public class CareerPage extends BasePage {
	...
	/**
	 * Robot class fallback (for native OS dialogs)
	 * Only used if sendKeys fails
	 */
	public void uploadFileUsingRobot(String path) throws AWTException {
		System.out.println("=== Using Robot Class for Upload ===");
		
		// First, try to click the upload button to open dialog
		try {
			if (!uploadInputOptions.isEmpty()) {
				uploadInputOptions.get(0).click();
				System.out.println("✓ Upload button clicked");
			}
		} catch (Exception e) {
			System.out.println("Could not click upload button, proceeding with Robot...");
		}
		
		Robot robot = new Robot();
		robot.delay(2000); // Wait for dialog to open
		...
		System.out.println("✓ File upload confirmed using Robot class");
