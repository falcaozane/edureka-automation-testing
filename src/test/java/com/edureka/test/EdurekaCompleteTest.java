...
public class EdurekaCompleteTest extends ReportManager {
	...
	public void navigateToWebinarsTest() throws Throwable {
		...
		Assert.assertTrue(
				driver.getCurrentUrl().contains("webinar") || webinarPageTitle.toLowerCase().contains("webinar"),
	...
	@Test(priority = 3, dataProvider = "webinarSearchTerms", dataProviderClass = TestDataProvider.class, description = "Search Webinars with Multiple Keywords", dependsOnMethods = "navigateToWebinarsTest")
	...
	public void navigateToCareersTest() throws Throwable {
		...
		Assert.assertTrue(
				driver.getCurrentUrl().contains("career") || careersPageTitle.toLowerCase().contains("career"),
	...
	@Test(priority = 7, dataProvider = "ApplyForJob", dataProviderClass = TestDataProvider.class, description = "Apply for Job with Test Data", dependsOnMethods = "navigateToJobOpeningTest")
	public void applyForJobTest(String applicantName, String applicantEmail, String applicantPhone) throws Throwable {
		...
		// ADDED: Extra wait after window switch
		Thread.sleep(3000);
		...
		// Upload resume and submit (NOW USING WORKING METHOD!)
		cp.uploadResumeAndSubmit();
		System.out.println("✓ Resume uploaded and application submitted!");

		Thread.sleep(3000);

		// Assertion: Verify submission completed
